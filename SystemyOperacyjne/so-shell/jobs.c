...
static void sigchld_handler(int sig) {
  ...
  sigfillset(&mask_all);
  while ((pid = waitpid(-1, &status, WNOHANG)) > 0) { // waiting for children
    Sigprocmask(SIG_BLOCK, &mask_all, &prev);
    for (int i = FG; i < njobmax; i++) {
      for (int j = 0; j < jobs[i].nproc; j++) { // updating proc status
        if (jobs[i].proc[j].pid == pid) {
          ...
          if (WIFSIGNALED(status)) {
            jobs[i].proc[j].exitcode = WTERMSIG(status);
            jobs[i].proc[j].state = FINISHED;
          }
      ...
      for (int j = 0; j < jobs[i].nproc; j++) { // updating job status
...
#ifdef STUDENT
    if (jobs[j].state == RUNNING && (which == ALL || which == RUNNING)) {
      msg("[%d] running '%s'\n", j, jobs[j].command);
    }
    if (jobs[j].state == STOPPED && (which == ALL || which == STOPPED)) {
      msg("[%d] suspended '%s'\n", j, jobs[j].command);
    }
    if (jobs[j].state == FINISHED && (which == ALL || which == FINISHED)) {
      if (exitcode(&jobs[j]) <= 1)
